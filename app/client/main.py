...
class loginWindow(QMainWindow):
    ...
    def funLogin(self):
        ...
        ret = ftp_client.login(textUserName, textPassWord)
        ...
        if not ret:
            ...
            self.close()
        clientDebug('loginIsSuccess = %d' % self.userInfo.isLoginSuccess)
...
class MyMainWidget(QMainWindow):
    def __init__(self, parent=None, userInfoObject=None):
        ...
        self.fileTree.setHeaderLabel('Remote')
...
def main():
    ...
    clientDebug('loginIsOK: %d, userName: %s, password: %s' % (user.isLoginSuccess, user.name, user.passWord))
    if not user.isLoginSuccess:
        # tftpSql.close()
        sys.exit()

    fileList = ftp_client.list()
