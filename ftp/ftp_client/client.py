...
from socket import *
from mysql.gen import *
from get import *
from put import *
from main_ftp import *
...
def doDownload(fileName, pgbar, button):
    pMemShare = Value('i', 0)
    Client.file_get(fileName, pMemShare)
...
def doupload(fileName, pgbar):
    def pgbarChange(x):
        pgbar.setValue(x)
    Client.file_put(fileName, pgbarChange)
...
class MyMainWidget(QMainWindow):
    ...
    def mySelectExtend(self):
        ...
        if getSelected:
            ...
            fileLs = "ls %s" % getChildNode
            print("+++",fileLs)

            filels = Client.list_folder(fileLs)
            print("=====",filels)
...
def main():
    ...
    Client.setFileRoot(main.userLocalDir)
    fileList = Client.file_list()

    # print(fileList)

    main.myInitAllItems(['root', '', '', ''], fileList)
    main.tabWidget.show()
    app.exec_()
